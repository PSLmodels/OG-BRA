...
## Schedule
For the October 21-25, 2024 United Nations `OG-BRA` training in Manila, we will be following the schedule in {numref}`table-schedule`.

```{list-table} UN OG-BRA 5-day training schedule
:header-rows: 1
:name: table-schedule
...
* - Mon.
  - Morning
  - Organizer introductions <br> [Setup Python](https://eapd-drb.github.io/OG-BRA/content/UNtutorial/getting_started.html#install-python), [Git, GitHub](https://eapd-drb.github.io/OG-BRA/content/UNtutorial/getting_started.html#installing-git-and-github), and [OG-BRA](https://eapd-drb.github.io/OG-BRA/content/UNtutorial/getting_started.html#fork-and-clone-og-bra-repository)
* -
  - Afternoon
  - Theory: ["Simple" 3-period-lived agent model](https://eapd-drb.github.io/OG-BRA/content/UNtutorial/3perOG.html)
* - Tue.
  - Morning
  - Review 3-period-lived-agent exercises (solutions in [this folder](https://github.com/EAPD-DRB/OG-BRA/blob/main/docs/book/content/UNtutorial/solutions/3perOG/)) <br> Review OG-Core and OG-BRA modules <br> Quick Git and GitHub workflow review
* -
  - Afternoon
  - Running OG-BRA, inputs, outputs <br> Calibrating OG-BRA, current state, still to do
* - Wed.
  - Morning
  - Running OG-BRA: Revisit some reforms from 2-day visit <br> Talk about new reforms, create project teams
* -
  - Afternoon
  - OG-BRA output: Tools to visualize/tabulate output <br> OG-BRA built-in calibration helps
* - Thu.
  - Morning
  - Calibrating OG-BRA: Issues and hot spots
* -
  - Afternoon
  - Calibrating OG-BRA: Issues and hot spots
* - Fri.
...
## Fork and clone OG-BRA repository
1. Forking the OG-BRA repository means that you are making a copy of that repository on your GitHub account in the cloud.
    - Go to the UN GitHub organization's main repository for OG-BRA (https://github.com/EAPD-DRB/OG-BRA).
    - In the upper-right area of the browser page, click the "Fork" button and select "Create fork". This will create an exact copy of the OG-BRA repository on your account. When you do this, you should see that the URL to the page has changed to `https://github.com/[YourGitHubHandle]/OG-BRA`.
...
2. The next step is to clone the repository from its current place in the cloud to your local computer's hard drive.
    ...
    - Copy the contents of your repository in the cloud to your hard drive by typing: `git clone https://github.com/[YourGitHubHandle]/OG-BRA.git`
    - Change directory to this new directory by typing: `cd OG-BRA`
    - Create an additional git remote named "upstream" that points to the main UN remote repository by typing: `git remote add upstream https://github.com/EAPD-DRB/OG-BRA.git`
...
## Create the ogbra-dev conda environment
...
If you have installed the Anaconda distribution of Python and you have cloned your OG-BRA fork of the repository to your local machine, you can create the `ogbra-dev` conda environment by doing the following steps:
- Open your terminal or command prompt and navigate to the OG-BRA repository folder on your hard drive.
- Type the following command: `conda env create -f environment.yml`
- Once the environment has been created, you must activate it: `conda activate ogbra-dev`
- In the activated `ogbra-dev` conda environment, install the `ogbra` Python package directly from your repository by typing: `pip install -e .`

Now you have the `ogbra` Python package installed in your `ogbra-dev` conda environment. Now you will be able to run the modules of the OG-BRA model from scripts and from Jupyter notebooks.

## Using Jupyter notebooks
A nice way to execute lines of code on your local computer is to use Jupyter notebooks. The `jupyter` package is installed as part of the `ogbra-dev` conda environment from the previous step. You can open a Jupyter notebook directly in VS Code, or you can open one from your terminal or command prompt.
...
- Navigate to the folder of the OG-BRA repository on your local machine.
- Activate the `ogbra-dev` conda environment by typing `conda activate ogbra-dev`.
...
- Either click the "New" button in the upper-right portion of the screen, or select "File" then "New" then "Notebook" from the menu at the upper-right. Make sure to select the `ogbra-dev` kernel.
