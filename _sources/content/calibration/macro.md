...
Aggregate (non-Social Security) transfers to households are set as a share of GDP with the parameter $\alpha_T$. We exclude Social Security from transfers since it is modeled specifically. With this definition, the share of transfers to GDP in 2023 is found to be 9.7% using [IMF data](https://www.imf.org/external/datamapper/profile/BRA).  The value found by differencing out government consumption expenditures (described below) from total government spending as report in the IMF data.
